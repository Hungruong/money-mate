...
const TradingScreen = () => {
  ...
  const fetchDashboardData = async () => {
    setLoading(true);
    try {
      const response = await fetch("https://query1.finance.yahoo.com/v7/finance/quote?symbols=AAPL,GOOGL,AMZN");
      const data = await response.json();
      setDashboardData(data.quoteResponse.result);
    } catch (error) {
  ...
  return (
    <View style={{ flex: 1, padding: 4, backgroundColor: "gray-100" }}>
      ...
      <View style={{ position: "absolute", bottom: 0, left: 0, right: 0, flexDirection: "row", justifyContent: "space-around", backgroundColor: "white", paddingVertical: 4, shadowColor: "black", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.25, shadowRadius: 4 }}>
        <TouchableOpacity style={{ padding: 12, backgroundColor: "blue", borderRadius: 8 }} onPress={() => navigation.navigate("Portfolio")}> 
          <Text style={{ color: "white" }}>User Portfolio</Text>
        </TouchableOpacity>
        <TouchableOpacity style={{ padding: 12, backgroundColor: "blue", borderRadius: 8 }} onPress={() => navigation.navigate("TransactionHistory")}> 
          <Text style={{ color: "white" }}>View Transactions</Text>
        </TouchableOpacity>
        <TouchableOpacity style={{ padding: 12, backgroundColor: "blue", borderRadius: 8 }} onPress={() => navigation.navigate("AutoTrade")}> 
          <Text style={{ color: "white" }}>Auto Trade</Text>
        </TouchableOpacity>
        <TouchableOpacity style={{ padding: 12, backgroundColor: "blue", borderRadius: 8 }} onPress={() => navigation.navigate("ManualTrade")}> 

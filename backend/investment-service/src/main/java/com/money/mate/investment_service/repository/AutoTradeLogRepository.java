...
public class AutoTradeLogRepository {
    ...
    private Map<String, AttributeValue> toAttributeMap(AutoTradeLog log) {
        ...
        item.put("sellPrice",
                log.getSellPrice() != null ? AttributeValue.builder().n(log.getSellPrice().toString()).build()
                        : AttributeValue.builder().nul(true).build());
        item.put("profitLoss",
                log.getProfitLoss() != null ? AttributeValue.builder().n(log.getProfitLoss().toString()).build()
                        : AttributeValue.builder().nul(true).build());
        ...
        item.put("tradeCompletedAt",
                log.getTradeCompletedAt() != null
                        ? AttributeValue.builder().s(log.getTradeCompletedAt().toString()).build()
                        : AttributeValue.builder().nul(true).build());
        return item;
    }
    ...
    private AutoTradeLog fromAttributeMap(Map<String, AttributeValue> item) {
        return AutoTradeLog.builder()
                ...
                .sellPrice(item.containsKey("sellPrice") && !item.get("sellPrice").nul()
                        ? new BigDecimal(item.get("sellPrice").n())
                        : null)
                .profitLoss(item.containsKey("profitLoss") && !item.get("profitLoss").nul()
                        ? new BigDecimal(item.get("profitLoss").n())
                        : null)
                .status(item.get("status").s())
                .tradeStartedAt(Instant.parse(item.get("tradeStartedAt").s()))
                .tradeCompletedAt(item.containsKey("tradeCompletedAt") && !item.get("tradeCompletedAt").nul()
                        ? Instant.parse(item.get("tradeCompletedAt").s())
                        : null)
                .build();
    }
}
